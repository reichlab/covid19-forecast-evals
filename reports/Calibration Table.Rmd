...
cum_calibration <- read_csv("../paper-inputs/20201021-cum-calibration.csv") %>%
  filter(location_name %in% datasets:: state.name)

inc_calibration <-  read_csv("../paper-inputs/20201021-inc-calibration.csv") %>%
  filter(location_name %in% datasets:: state.name)
```
...
cum_scores_merge <- cum_scores %>%
  ...
  select(model, unit, target, truth, location_name, target_end_date_1wk_ahead, quantile, value) %>%
...
kableExtra::kable(calibration_scores_cum_table) %>%
