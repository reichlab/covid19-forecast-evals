...
forecasts_case_x5 <- forecasts_case_all %>%
  filter(forecast_date > mondays[20] & forecast_date <= mondays[25])

forecasts_case_x6 <- forecasts_case_all %>%
  filter(forecast_date > mondays[25] & forecast_date <= mondays[30])

forecasts_case_x7 <- forecasts_case_all %>%
  filter(forecast_date > mondays[30] & forecast_date <= mondays[35])

forecasts_case_x8 <- forecasts_case_all %>%
  filter(forecast_date > mondays[35] & forecast_date <= mondays[40])

forecasts_case_x9 <- forecasts_case_all %>%
  filter(forecast_date > mondays[40] & forecast_date <= mondays[45])

forecasts_case_x10 <- forecasts_case_all %>%
  filter(forecast_date > mondays[45])
...
forecasts_case_x6_update <- unique(forecasts_case_x6)
forecasts_case_x7_update <- unique(forecasts_case_x7)
forecasts_case_x8_update <- unique(forecasts_case_x8)
forecasts_case_x9_update <- unique(forecasts_case_x9)
forecasts_case_x10_update <- unique(forecasts_case_x10)
...
forecasts_case_update <- rbind(forecasts_case_x1_update,forecasts_case_x2_update,forecasts_case_x3_update,forecasts_case_x4_update,forecasts_case_x5_update,
                              forecasts_case_x6_update,forecasts_case_x7_update,forecasts_case_x8_update,forecasts_case_x9_update,forecasts_case_x10_update)
...
# DEATHS
# Load truth data
# RESTART RSTUDIO BEFORE RUNNING CODE FOR DEATHS
###########################
...
#query forecast data from zoltar for past 12 month submission weeks. 
forecasts_inc1 <- forecasts_inc_function("1","10")
forecasts_inc2 <- forecasts_inc_function("11","20")
forecasts_inc3 <- forecasts_inc_function("21","30")
forecasts_inc4 <- forecasts_inc_function("31","40")
forecasts_inc5 <- forecasts_inc_function("41","52")

forecasts_inc <- rbind(forecasts_inc1,forecasts_inc2,forecasts_inc3,forecasts_inc4,forecasts_inc5) %>% filter(model %in% models_primary_secondary)
...
save(forecasts_inc_update, file = "reports/forecasts_inc_update.rda")

#covidhub utils function to score the data
load(file = "reports/forecasts_inc_update.rda") 
load(file = "reports/truth_dat_inc.rda") 
...
#reduce size
memory.limit(30000)
load(file = "reports/forecasts_hosp_x.rda")
