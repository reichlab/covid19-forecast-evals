...
plot_by_location_wis <- function(df) {
scores <- df %>%
...
average_by_loc_to_plot$model <- reorder(average_by_loc_to_plot$model, -average_by_loc_to_plot$log_relative_wis)
...
```{r}
wis_plot_function <- function(x,y) {
wis_plot <- x %>% 
  filter(target_end_date >= first_eval_sat) %>%
...
model_levels <- wis_plot %>%
  group_by(model) %>%
  summarise(sum_val = sum(mean_values)) %>% arrange(desc(sum_val)) %>% pull(model) 
...
```{r wis bar function, fig.height= 10, fig.width=6}
wis_plot_function(x = score_case_all, y = score_case_10wks)
```
...
plot_by_location_wis(score_case_10wks)
```
...
wis_plot_function(score_inc_all, score_inc_10wks)
```
...
### Evaluation by WIS
```{r, width=15, fig.height=11}
plot_by_location_wis(score_inc_10wks)
```
