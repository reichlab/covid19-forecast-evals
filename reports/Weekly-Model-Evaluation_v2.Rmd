...
n_weeks_eval <- 8 #weeks included in evaluation
```
...
# write.csv(truth_dat_inc, "truth_dat_cum.csv", row.names = FALSE)
```
...
truth_dat_cum <- read.csv("truth_dat_cum.csv") %>% mutate(target_end_date = as.Date(target_end_date))
```
...
# write.csv(score_cum_edit, "score_cum_edit.csv", row.names = FALSE)
```
...
score_case_edit <- read.csv("score_case_edit.csv") %>% mutate(target_end_date = as.Date(target_end_date),
                                                                         forecast_date = as.Date(forecast_date),
                                                                         submission_sat = as.Date(submission_sat))
score_inc_edit <- read.csv("score_inc_edit.csv") %>% mutate(target_end_date = as.Date(target_end_date),
                                                                         forecast_date = as.Date(forecast_date),
                                                                        submission_sat = as.Date(submission_sat))
score_cum_edit <- read.csv("score_cum_edit.csv") %>% mutate(target_end_date = as.Date(target_end_date),
                                                                         forecast_date = as.Date(forecast_date),
submission_sat = as.Date(submission_sat))
```
...
For inclusion in this table, a team must have submitted a model for at least 4 out of the last `r n_weeks_eval` weeks. A model was counted if it included at least 25 locations and forecasts for 1 - 4 week ahead horizons. 
...
by_week_wis_1wk <- ggplot(data =  byweek_case %>% filter(horizon == "1"), aes(x = submission_sat, y = mean_wis, color = model)) +
...
by_week_wis_4wk <- ggplot(data =  byweek_case %>% filter(horizon == "4"), aes(x = submission_sat, y = mean_wis, color = model)) +
  geom_line(aes(group = model), alpha=.5) +
  geom_point(aes(group = model), alpha=.5, size = 2) +
  expand_limits(y=0) +
  scale_y_continuous(name = "Average WIS") +
  ...
  theme(axis.ticks.length.x = unit(0.5, "cm"),
  axis.text.x = element_text(vjust = 7, hjust = -0.2)) + 
  geom_line(data = meanwis_1wk, aes(x = submission_sat, y = mean_wis), alpha=.5, color = "black", linetype = 2) +
  geom_point(data = meanwis_1wk, aes(x = submission_sat, y = mean_wis), alpha=.5, size = 2, color = "black") 
...
For inclusion in this table, a team must have submitted a model for at least 4 weeks out of the last `r n_weeks_eval` weeks and have submitted a forecast for every horizon (1 - 4 weeks ahead) for each week. 
```{r}
#calibration table 
...
by_week_wis_1wk <- ggplot(data =  byweek_inc %>% filter(horizon == "1"), aes(x = submission_sat, y = mean_wis, color = model)) +
  geom_line(aes(group = model), alpha=.5) +
  geom_point(aes(group = model), alpha=.5, size = 2) +
  expand_limits(y=0) +
  scale_y_continuous(name = "Average WIS") +
...
###4 Week Horizon
```{r,fig.width=10, fig.height=6}
by_week_wis_4wk <- ggplot(data =  byweek_inc %>% filter(horizon == "4"), aes(x = submission_sat, y = mean_wis)) +
...
The number of models who submitted forecasts for cumulative deaths is `r n_unique_models_cum`. The number of models that submitted forecasts for all `r n_weeks_eval` weeks was `r n_models_all_weeks`. The number of teams that submitted forecasts for all 57 locations was `r n_models_all_locations`.
...
```{r}
#calibration table 
cum_calibration <- calib_table(score_cum_edit)
```
...
scores <- inc_scores %>% filter(horizon %in% paste(1:4), 
                                location %in% the_locations,
                                score_name %in% c("abs_error", "wis")) %>%
  select("model", "timezero", "location", "horizon", "score_name", "score_value") %>%
  pivot_wider(names_from = score_name, values_from = score_value)
...
by_week_wis_1wk <- ggplot(data =  byweek_cum %>% filter(horizon == "1"), aes(x = submission_sat, y = mean_wis, color = model)) +
  geom_line(aes(group = model), alpha=.5) +
  geom_point(aes(group = model), alpha=.5, size = 2) +
  expand_limits(y=0) +
  scale_y_continuous(name = "Average WIS") +
...
###4 Week Horizon
```{r,fig.width=10, fig.height=6}
by_week_wis_4wk <- ggplot(data =  byweek_cum %>% filter(horizon == "4"), aes(x = submission_sat, y = mean_wis, color = model)) +
  geom_line(aes(group = model), alpha=.5) +
  geom_point(aes(group = model), alpha=.5, size = 2) +
  expand_limits(y=0) +
  scale_y_continuous(name = "Average WIS") +
