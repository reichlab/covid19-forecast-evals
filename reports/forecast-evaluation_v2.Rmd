...
```{r Figure 4: models aggregated by target and location, fig.height= 8, fig.width= 10}
...
ggplot(avg_byweek_calc, aes(x= lubridate::ymd(first_fcast_sat), y=MAE, color = model_code, fill = model_code)) +
...
#Plot of MAE stratified by submission week
ggplot(avg_byweek_calc_inc, aes(x= lubridate::ymd(first_fcast_sat), y=MAE, color = model_code, fill = model_code)) +
  scale_x_date(date_labels = "%Y-%m-%d", breaks = c(avg_byweek_calc_inc$first_fcast_sat)) + 
  geom_line() + 
  colScale_cum +
  geom_point(size = 2) + 
  xlab("1 Week Ahead Target Date") + ylab(NULL) + 
...
#Figure 4: Relative MAE
ggplot(avg_byweek_calc_inc, aes(x= lubridate::ymd(first_fcast_sat), y=relative_MAE, color = model_code, fill = model_code)) +
  scale_x_date(date_labels = "%Y-%m-%d", breaks = c(avg_byweek_calc_inc$first_fcast_sat)) +
  #scale_color_manual(values = colors2) +
   geom_line() + 
  colScale_cum +
...
#Figure 4: average WIS
ggplot(avg_byweek_calc_inc, aes(x= lubridate::ymd(first_fcast_sat), y=avg_wis, color = model_code, fill = model_code)) +
  scale_x_date(date_labels = "%Y-%m-%d", breaks = c(avg_byweek_calc_inc$first_fcast_sat)) + 
  geom_line() + 
  colScale_cum +
  geom_point(size = 2) + 
...
#Figure 4: relative WIS
ggplot(avg_byweek_calc_inc, aes(x= lubridate::ymd(first_fcast_sat), y=relative_WIS, color = model_code, fill = model_code)) +
  scale_x_date(date_labels = "%Y-%m-%d", breaks = c(avg_byweek_calc_inc$first_fcast_sat)) +
   geom_line() + 
  colScale_cum +
  geom_point(size = 2, value = colors) + 
