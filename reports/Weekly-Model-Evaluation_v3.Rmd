...
new_dates_scoreinc <- score_inc_all %>%
  filter(target_end_date >= as.Date("2021-02-14"))
new_dates_scorehosp <- score_hosp_all %>%
  filter(target_end_date >= as.Date("2021-02-14"))
...
score_hosp_all <- rbind(new_dates_scorehosp, old_dates_scorehosp)

truth_dat_case <- rbind(new_truth_case,old_truth_case)
truth_dat_inc <- rbind(new_truth_inc, old_truth_inc)
truth_dat_hosp <- rbind(new_truth_hosp, old_truth_hosp)
```
...
* The **weighted interval score (WIS)** is a proper score that combines a set of interval scores for probabilistic forecasts that provide quantiles of the predictive forecast distribution. To account for the variation in difficult of forecasting different weeks and locations, a pairwis approach was used to calculated the relative adjusted WIS. The code for this comparison can be found [here](https://github.com/jbracher/pairwise_comparisons). A preprint on this method for calculating the WIS can be found [here](https://arxiv.org/pdf/2005.12881.pdf). Models with an adjusted relative WIS  lower than 1 are more accurate than the baseline, and models with an adjusted relative WIS greater than 1 are less accurate than the baseline is predicting the number of incident Hospitalizations. 

* The **mean absolute error** is defined as the the difference between the forecasted value and the actual value. For this calculation, point forecasts submitted by teams were used. When point forecasts were not available, the 0.50 quantile was used. Similar to calculating the WIS, a pairwise approach was used to account for variation across locations and weeks. Models with an adjusted relative MAE lower than 1 are more accurate than the baseline, and models with an adjusted relative MAE greater than 1 are less accurate than the baseline is predicting the number of incident Hospitalizations. 
-->
...
```{r relative wis Hospitalization}
#at least 50% of recent WIS or 50% of recent MAE
hosp_accuracy_recent <- recent_accuracy_filter(score_hosp_all)
...
leaderboard_table <- hosp_count_fcasts_recent %>%
  full_join(hosp_count_fcasts_historical) %>%
...
```{r}
hosp_model_order <- leaderboard_table %>%
...
#calibration table 
hosp_calib <- calib_table(score_hosp_all)
hosp_calib
```
...
This figure shows the number of incident reported COVID-19 hosps reported each week in the US. The period between the vertical blue line and the black shows the weeks included in the "recent" model evaluations. The period between the vertical red line and the black shows the weeks included in the "historical" model evaluations.

```{r, fig.width=8, fig.height=5}
truth_US_hosp <- truth_dat_hosp_all %>%
  filter(location == "US")

plot_truth(dat = truth_US_hosp, tar="hosps")
```
...
byweek_hosp <- by_week_function(score_hosp_all, var = "wis")
# find 97.5 percentile
b<-byweek_hosp %>%
  filter(horizon == "4") 
p975<-quantile(b$mean_score,probs=.975, na.rm = TRUE)

by_week_wis_1wk <- plot_byweek_function(byweek_hosp, var = "wis", horizon_num = "1") + coord_cartesian(ylim=c(0, p975))
...
by_week_wis_4wk <- plot_byweek_function(byweek_hosp, var = "wis", horizon_num = "4") +
...
```{r,fig.width=10, fig.height=6}
byweek_hosp <- by_week_function(score_hosp_all, var = "coverage_95")
by_week_wis_1wk <- plot_byweek_function(byweek_hosp, var = "coverage_95", horizon_num = "1") +
...
```{r,fig.width=10, fig.height=6}
by_week_wis_4wk <- plot_byweek_function(byweek_hosp, var = "coverage_95", horizon_num = "4") +
  geom_hline(yintercept = .95)
