...
inc_tmp_unique <-  inc_tmp %>%
  mutate(sat_fcast_week = get_next_saturday(timezero) + ifelse(wday(timezero)<=2,0,7) - 7) %>%
...
for_loc_figure <- num_loc %>%
  ...
  filter(min(sat_fcast_week) <= as.Date("2020-08-29")) %>% #filter models that have 
  filter(!(model %in% c( "CU-nochange", "CU-scenario_high", "CU-scenario_low", "CU-scenario_mid")))
```
