...
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
  inc_coverage_recent_models <- score_inc_all_ohio  %>%     
...
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
inc_coverage_recent <- recent_coverage_filter(score_inc_all_ohio,inc_coverage_recent_models)
...
inc_countc_fcasts_historical <-  inc_coverage_historical %>% group_by(model,score_name) %>%
  ...
  recent_calib_50  <- score_inc_all_ohio  %>%
    filter(model %in% inc_coverage_recent_models) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
...
#filter for inclusion in recent coverage table 
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
  hosp_coverage_recent_models <- score_hosp_all  %>%     
    filter(target_end_date >= first_eval_sat) %>% 
    filter(n_weeks_10wksPrior >= 5 |  n_weeks_3wksPrior >= 2)  %>% 
    group_by(model)  %>% 
   summarize(Freq=n()) %>% 
...
#Filtered to include only models in scoreboard (coverage table) 
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
hosp_coverage_recent <- recent_coverage_filter(score_hosp_all,hosp_coverage_recent_models)
...
hosp_countc_fcasts_historical <-  hosp_coverage_historical %>% group_by(model,score_name) %>%
  ...
  recent_calib_50  <- score_hosp_all  %>%
    filter(model %in% hosp_coverage_recent_models) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
     filter(target_end_date >= first_eval_sat) %>%  droplevels() %>%
    droplevels() %>%
...
#Filtered to include only models in scoreboard (coverage table) 
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
case_coverage_recent_models <- score_case_all_s %>%
    filter(target_end_date >= first_eval_sat) %>% 
    filter(n_weeks_10wksPrior >= 5 |  n_weeks_3wksPrior >= 2)  %>% 
    group_by(model)  %>% 
   summarize(Freq=n()) %>% 
...
case_coverage_recent <- recent_coverage_filter(score_case_all_s,case_coverage_recent_models)
...
case_countc_fcasts_historical <-  case_coverage_historical %>% group_by(model,score_name) %>%
  ...
  filter(location %in% the_locations)  %>% #states, us and territories
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
    select(model,horizon,score_value,n_weeks)  %>%
  ...
  recent_calib_50  <- score_case_all_s  %>%
    filter(model %in% case_coverage_recent_models) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
    filter(target_end_date >= first_eval_sat) %>%  droplevels() %>%
    select(model,horizon,score_value)  %>%
...
### 1 Week Horizon WIS
```{r,fig.width=10, fig.height=6}
byweek_case <- by_week_function(score_case_all_s, var = "wis")
# find 97.5 percentile
b<-byweek_case %>%
  filter(horizon == "4") 
p975<-quantile(b$mean_score,probs=.975, na.rm = TRUE)
...
```{r,fig.width=10, fig.height=6}
byweek_case <- by_week_function(score_case_all_s, var = "coverage_95")
by_week_wis_1wk <- plot_byweek_function(byweek_case, var = "95% PI Coverage", horizon_num = "1") +
  geom_hline(yintercept = .95)
ggplotly(by_week_wis_1wk,tooltip = c("label", "labelx", "labely"))
```
...
#filter for inclusion in recent coverage table 
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
  case_coverage_recent_models <- score_case_all_c  %>%     
    filter(target_end_date >= first_eval_sat) %>% 
    filter(n_weeks_10wksPrior >= 5 |  n_weeks_3wksPrior >= 2)  %>% 
    group_by(model)  %>% 
   summarize(Freq=n()) %>% 
...
#Filtered to include only models in scoreboard (coverage table) 
#at least 5 weeks overall or at least 2 out of the last 3 weeks 
case_coverage_recent <- recent_coverage_filter(score_case_all_c, case_coverage_recent_models)
...
case_countc_fcasts_historical <-  case_coverage_historical %>% group_by(model,score_name) %>%
  ...
  historical_coverage <-  score_case_all_c  %>% 
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50"))
...
In the following figures, we have evaluated models across multiple forecasting weeks. Points included in this comparison are for models that have submitted probabilistic forecasts for all 500 counties. 
...
### 1 Week Horizon WIS
```{r,fig.width=10, fig.height=6}
byweek_case <- by_week_function(score_case_all_c, var = "wis")
# find 97.5 percentile
b<-byweek_case %>%
  filter(horizon == "4") 
p975<-quantile(b$mean_score,probs=.975, na.rm = TRUE)
...
```{r,fig.width=10, fig.height=6}
byweek_case <- by_week_function(score_case_all_c, var = "coverage_95")
by_week_wis_1wk <- plot_byweek_function(byweek_case, var = "95% PI Coverage", horizon_num = "1") +
  geom_hline(yintercept = .95)
ggplotly(by_week_wis_1wk,tooltip = c("label", "labelx", "labely"))
```
