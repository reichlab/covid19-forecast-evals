...
# remove COVIDhub-4_week_ensemble model from cases and deaths and hosp for now 
...
Scores are aggregated separately for the most recent 10 weeks and for 26 historical weeks. Since hospitalizations are forecast across daily horizons, means are calculated for each week.
...
inc_scores_wk <- inc_scores %>%
  rename(horz_day = horizon)  %>%
  mutate(horizon = ceiling(horz_day/7))   %>%
 group_by(model,score_name,location,timezero,horizon) %>% summarize(score_value_wk = mean(score_value)) 
  
# restrict to 1-4 wk ahead state-level
scores <- inc_scores_wk %>% filter(horizon %in% paste(1:4)) %>%
  filter(score_name %in% c("wis","abs_error")) %>%
  select("model", "timezero", "location", "horizon", "score_name", "score_value_wk") %>%
  pivot_wider(names_from = score_name, values_from = score_value_wk) %>%
  filter(!is.na(wis)) %>%
  droplevels()
...
#change horizon to weekly
score_hosp_all_hor_wk <- score_hosp_all %>%
  rename(horz_day = horizon)  %>%
  mutate(horizon = ceiling(horz_day/7))   

  historical_calib_50 <-  score_hosp_all_hor_wk  %>% 
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
    select(model,horizon,score_value,n_weeks)  %>%
  ...
  recent_calib_50  <- score_hosp_all_hor_wk  %>%
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_50")) %>%
    filter(target_end_date >= first_eval_sat) %>%  droplevels() %>%
  ...
  historical_calib_95 <-  score_hosp_all_hor_wk  %>% 
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_95")) %>%
    select(model,horizon,score_value)  %>%
  ...
  recent_calib_95  <- score_hosp_all_hor_wk  %>%
    filter(n_weeks >= 5 |  n_weeks_3wksPrior >= 2) %>% 
    filter(!is.na(score_value))  %>%
    filter(score_name %in% c("coverage_95")) %>%
    filter(target_end_date >= first_eval_sat) %>%  droplevels() %>%
...
For the first 2 figures, WIS is used as a metric. The first figure shows the mean WIS across all 50 states for submission weeks beginning `r format(first_eval_sat_hist, "%B %d, %Y")` at a 1 week horizon. The second figure shows the mean WIS aggregated across locations, however it is for a 4 week horizon.  Average daily hospitalizations across specificed horizon are displayed.
...
byweek_case <- by_week_function(score_hosp_all_wk, var = "wis")
# find 97.5 percentile
b<-byweek_case %>%
  filter(horizon == "4") 
p975<-quantile(b$mean_score,probs=.975, na.rm = TRUE)
...
```{r,fig.width=10, fig.height=6}
byweek_case <- by_week_function(score_hosp_all_wk, var = "coverage_95")
by_week_wis_1wk <- plot_byweek_function(byweek_case, var = "95% PI Coverage", horizon_num = "1") +
  geom_hline(yintercept = .95)
ggplotly(by_week_wis_1wk,tooltip = c("label", "labelx", "labely"))
```
...
This figures below show recent model performance stratified by location. We only included forecasts for the last 10 weeks. Models were included if they had submitted forecasts for all 4 horizons and submitted  at least 50% of forecasts during this time.   Locations are sorted by cumulative death counts.
