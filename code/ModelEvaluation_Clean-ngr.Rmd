...
## stan model files
stan_model_ae <- readRDS("../data-inputs/20200927-stan-fit-scores-negbin.rds")
...
```{r zoltar-connection-and-data, cache=FALSE, echo=FALSE, include=FALSE, message=FALSE,results="hide"}
...
the_locations <- zoltar_units(zoltar_connection, project_url = "https://zoltardata.com/api/project/44/")$name %>% 
  magrittr::extract(. %in% fips_scoring$location)

the_targets_cum <- paste(1:20, "wk ahead cum death")

the_targets_inc <- paste(1:4, "wk ahead inc death")
...
#Cumulative dataset 
cum_tmp <- do_zoltar_query(zoltar_connection, 
