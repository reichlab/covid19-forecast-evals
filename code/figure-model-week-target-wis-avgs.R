...
#Figure 4: by week, target, and colored by model Average WIS over time 

avg_wis_by_model_target_week %>%
  group_by(model, unit) %>%
  summarise(n = n())

baseline_avg_week <- inc_scores %>%
  filter(model=="COVIDhub-baseline") %>%
  filter(!(location_name %in% c("United States", "American Samoa", "Guam", "Northern Mariana Islands", "Virgin Islands", "Puerto Rico", "District of Columbia"))) %>%
  group_by(target) %>%
...
## by target and week
inc_targetweek <- inc_scores %>%
  filter(model!="COVIDhub-baseline") %>%
  group_by(target, target_end_date_1wk_ahead) %>%
  summarize(median_wis = median(wis, na.rm = TRUE), mean_wis = mean(wis, na.rm = TRUE))

f4 <- ggplot(avg_wis_by_model_target_week, aes(x=target_end_date_1wk_ahead, y=mean_wis)) +
  #geom_hline(yintercept=ensemble_1wk_avg, linetype=2)+
  geom_hline(data=baseline_avg_week, aes(yintercept=mean_wis), linetype=2) +
  geom_point(aes(color=model), position=position_jitter(width=.2, height=0), alpha=.8) +
  ##geom_point(aes(color=obs_exp_locs, fill=target_end_date_1wk_ahead), shape=21, position=position_jitter(width=.2, height=0), alpha=.8) +
  geom_point(data=avg_wis_by_week_target, shape=4, color="#d95f02", size=2, stroke=1) +
  theme_bw() +
  theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1), legend.position = c(.75, .9), legend.direction = "horizontal") +
  facet_wrap(.~target) + #, scales="free_y") +
  ylab("Average WIS") + xlab(NULL) +
  ...
  scale_fill_date(name="forecast date") +
  scale_y_continuous(breaks=seq(0, 400, by=50)) +
  scale_x_date(date_labels = "%Y-%m-%d", breaks = c(avg_wis_by_model_target_week$target_end_date_1wk_ahead)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
        axis.text.y = element_text(size = 10)) 


pdf(file = "figures/model-target-week-fig4-wis-avgs.pdf", width=8, height=6)
print(f4)
dev.off()

jpeg(file = "figures/model-target-week-fig4-wis-avgs.jpg", width=8, height=6, units="in", res=200)
print(f4)
dev.off()
